...
line-harness/
├── apps/
│   ├── worker/        # Cloudflare Workers API サーバー
│   ├── web/           # Next.js 管理パネル
│   └── liff/          # LIFF フロントエンド (Vite)
├── packages/
...
LIFF アプリは `apps/liff/` にあり、Vite でビルドされる。
...
# Cloudflare Pages や Vercel にデプロイ
```

### LIFF 設定

1. LINE Developers Console で LIFF アプリを作成
2. エンドポイント URL にデプロイ先を設定
3. `LIFF_URL` 環境変数に LIFF URL を設定
