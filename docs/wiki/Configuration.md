...
Lステップと同じく、外部ブラウザではすぐにWebログインへ送らず、LINEアプリを開く
ゲート画面を表示します。LIFFアプリの `botPrompt` は `none` に設定してください。
友だち状態は `liff.getFriendship()` ではなく、署名検証済みIDトークンと
L HarnessがWebhookで保持する `friends.is_following` から判定します。そのため、
LINE Loginチャネルの「リンクされたLINE公式アカウント」は必須ではありません。

iOSではLINEアプリ起動の互換手段として `line://app/...` も使用しますが、これは
LINE公式では非推奨です。画面には通常のLIFF URLによるアプリ起動とブラウザログインも
フォールバックとして残し、OS側の挙動変更時にも利用者が進める構成にします。
