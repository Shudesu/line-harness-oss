...
- 配信の送信元が不明・無効・未設定の場合の、既定トークンへの代替を停止
- 部分失敗・受付結果不明・受付後の記録保存失敗を「要確認」として表示
- 保存失敗による即時配信の再送可能状態への巻き戻しを防止
- キューの失敗理由と受付件数を保持し、失敗応答後の画面を更新

登録済み環境の送信元未指定配信は、更新前に送信元と既配信の有無を確認してください。[更新手順と注意点](docs/release-notes/v0.24.2.md)
